...
const airlinesPro = () => {
  const admin = () => {
    let adminActionPrompt = prompt(
      "Teclea 1 para crear más vuelos y 2 para eliminarlos."
    );
    ...
    while (adminActionPrompt !== "1" && adminActionPrompt !== "2") {
      adminActionPrompt = prompt(
        '¡Vaya! Ha habido un error. Por favor, introduce "1" o "2" como respuesta.'
      );
    }

    while (flights.length === 16 && adminActionPrompt === "1") {
      alert(
        "Has alcanzado el máximo de vuelos permitidos. Debes eliminar vuelos para poder continuar añadiendo más."
      );
      admin();
    }
    ...
    if (adminActionPrompt === "1") {
      const createNewFlight = () => {
        ...
        let scalePrompt = prompt(
          '¿El vuelo tiene escala? Introduce "True" o "False".'
        ).toLowerCase();
        ...
        let moreFlightsPrompt = prompt(
          '¿Quieres crear más vuelos? Introduce "Sí" o "No".'
        ).toLowerCase();
    ...
    if (adminActionPrompt === "2") {
      let flightId = +prompt(
        "Por favor, introduce el ID del vuelo que quieras eliminar"
      );
  ...
  const user = () => {
    ...
    if (equalPriceFlights.length !== 0) {
      console.log(
        `Vuelos a ${pricePrompt}€ o menos: \n${equalPriceFlights.join("\n")}`
      );
    } else {
      console.log(
        "Lo sentimos, a ese precio no tenemos ningún vuelo disponible."
      );
    }

    let searchMorePrompt = String(
      prompt('¿Quieres buscar más vuelos? Introduce "Sí" o "No".')
    ).toLowerCase();
  ...
  const askRoleQuestion = () => {
    ...
    if (roleAnswer === null) {
      alert(
        '¡Vaya! Ha habido un error. Por favor, introduce "ADMIN" o "USER" como respuesta.'
      );
      return askRoleQuestion();
    } else if (
      roleAnswer.toLowerCase() !== "admin" &&
      roleAnswer.toLowerCase() !== "user"
    ) {
      alert(
        '¡Vaya! Ha habido un error. Por favor, introduce "ADMIN" o "USER" como respuesta.'
      );
